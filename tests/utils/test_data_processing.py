...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "2020-12-31T14:00:00",
            1.49,
        ),  # Expected average temperature for 1 Month
        (
            "Monthly",
            "pres",
            "2020-12-01T14:00:00",
            "2020-12-31T14:00:00",
            982.52,
        ),  # Expected average pressure for 1 Month
        (
            "Monthly",
            "vel",
            "2020-12-01T14:00:00",
            "2020-12-31T14:00:00",
            2.73,
        ),  # Expected average velocity for 1 Month
    ],
)
...
def test_timezone_handling(mock_data):
    ...
    result_naive = aggregate_data(
        df_naive, "Daily", "2020-12-01T14:00:00", "2020-12-31T14:00:00"
    )
    result_aware = aggregate_data(
        df_aware, "Daily", "2020-12-01T14:00:00", "2020-12-31T14:00:00"
    )
...
def test_missing_columns(mock_data):
    ...
    try:
        result = aggregate_data(
            df, "Daily", "2020-12-01T00:00:00", "2020-12-31T00:00:00"
        )
        assert (
            "temp" not in result.columns
        ), "The 'temp' column should not be in the result if it's missing in the input."
        assert (
            len(result) > 0
        ), "The result should not be empty when other columns are present."
...
def test_single_row_dataframe():
    ...
    result = aggregate_data(
        single_row, "Daily", "2020-12-01T00:00:00", "2020-12-31T00:00:00"
    )
    assert len(result) == 1
...
def test_nan_handling(mock_data):
    ...
    assert len(result) > 0
    assert (
        result["temp"].notna().all()
    )  # Ensure all remaining 'temp' values are non-NaN
...
def test_timezone_conversion(mock_data):
    ...
    df["fhora"] = pd.to_datetime(df["fhora"], errors="coerce").dt.tz_localize(
        None
    )  # Ensure naive first
    ...
    assert len(result) > 0
    assert (
        result["fhora"].iloc[0].tz.zone == "Europe/Berlin"
    )  # Ensure timezone is correctly converted
